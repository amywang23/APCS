// Name: Amy Wang
// Date: 1/27/2021 || 3/26/2021

import java.util.PriorityQueue;

public class LunchRoomJan
{
   public static final int TIME = 1080;  //18 hrs * 60 min
   public static double CHANCE_OF_CUSTOMER = .2;
   public static String[] gradeNames = {"Senior", "Junior", "Sophomore", "Freshman"};
	public static int[] customers = new int[4];
	public static double[] totalMinutes = new double[4];
	public static int[] longestWaitTime = new int[4];
...
//	public static PrintWriter outfile = null; // file to display the queue information
   ...
   public static void main(String[] args)
   {
      ...
      /* report the data  */ 
      for(int i = 0; i< 4; i++) {
    	  System.out.println(gradeNames[i] + "   \t\t" + customers[i] + "\t\t" + longestWaitTime[i] + "\t\t" + totalMinutes[i]/customers[i]);
      }
   }
   
   public static void cafLunch(int TIME, PriorityQueue<Customer> q)
	{
		Customer c = null;
		thisCustomersTime = 0;
		int grade = 0;
		...
		for(int i = 1; i <= TIME; i++) {
			if(Math.random() <= CHANCE_OF_CUSTOMER) {
				...
				totalCustomers++;
				customers[grade]++;
				c = new Customer(i, timeToOrderAndBeServed(), totalCustomers, grade);
			...
			if(!q.isEmpty()) { //if there are customer(s) in queue
				if(serviceWindow == 0) { //service window open
					...
					if(thisCustomersTime == 1) {
						grade = q.peek().getGrade();
						int waitingTime = i - q.peek().getArrivedAt();
						totalMinutes[grade] += waitingTime;
						if(longestWaitTime[grade] < waitingTime) {
							longestWaitTime[grade] = waitingTime;
						}
						q.remove();
						if(!q.isEmpty()) {
							thisCustomersTime = q.peek().getOrderAndBeServed();
...
class Customer implements Comparable<Customer>
{
	...
	public Customer(int a, int b, int c, int g) {
		arrivedAt = a;
		orderAndBeServed = b;
		customerNum = c;
		grade = g;
	...
	public String toString(){
		return customerNum + ":" + LunchRoomJan.gradeNames[grade].substring(0, 2);
	}
	...
	public int compareTo(Customer o) {
		if(this.grade == o.grade)
			return this.getCustomerNum()-o.getCustomerNum();
