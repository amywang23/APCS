...
class DocumentIndex extends TreeMap<String, TreeSet<Integer>>
{
	...
	public void addWord(String word, int lineNum)
	{
		if(!word.trim().isEmpty()) {
			...
			if(this.containsKey(newWord)) {
				this.get(newWord).add(lineNum);
			} else {
				TreeSet<Integer> set = new TreeSet<Integer>();
				set.add(lineNum);
				this.put(newWord, set);
			}
		}
	}
	...
	{
		String s = "";
		for (Map.Entry<String,TreeSet<Integer>> entry : this.entrySet()) {
			s = s + entry.getKey() + " " + entry.getValue().toString().substring(1, entry.getValue().toString().length()-1) + "\n";
		}
		return s;
	}
}
